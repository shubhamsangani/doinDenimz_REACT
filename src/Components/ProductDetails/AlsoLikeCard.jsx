...
function AlsoLikeCard({ filteredProductDetail }) {
  const { status, favoriteProducts } = useSelector((state) => state.API);
  const alsoLikeProduct = filteredProductDetail?.filter(
    (item) => item.is_product_high_seller === "Yes"
  );
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
          {status === STATUSES.LOADING
            ...
            : alsoLikeProduct.slice(0, 4).map((item, index) => {
